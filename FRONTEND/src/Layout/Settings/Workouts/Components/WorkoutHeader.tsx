...
function WorkoutHeader(props: { title: string; totalRecords: number; handleOpen: () => void }) {
  const { request } = useRequest();
  const exportToCSV = async () => {
    const moduleName = 'workout-types';
        ...
        <div className={styles.SubHead}>
          ...
          <div>
            ...
            <Button className="customBtn SmBtn" onClick={() => props.handleOpen()}>
              <Image alt="add" height={16} width={16} src="/assets/icons/add.svg" /> Add a new workout
            </Button>
          </div>
        </div>
      </div>
