...
function NutritionHeader(props: { title: string; totalRecords: number; handleOpen: () => void }) {
  const { request } = useRequest();
  const exportToCSV = async () => {
    const moduleName = 'nutrition-types';
        ...
        <div className={styles.SubHead}>
          ...
          <div>
            ...
            <Button className="customBtn SmBtn" onClick={() => props.handleOpen()}>
              <Image alt="add" height={16} width={16} src="/assets/icons/add.svg" /> Add a new nutrition
            </Button>
          </div>
        </div>
      </div>
