...
import { Button, Typography, Container, Divider, Grid, Drawer, Box, IconButton, useTheme, Tooltip } from '@mui/material';
...
function Index() {
  ...
  const removeUser = async (id: string | string[]) => {
    if (!id?.length) return;
    const confirm = await deleteDialog('Are you sure you want to delete the Client?', 'Delete Client');
    if (confirm) {
      const res = (await request('removeUserFromList', { id: id })) as REQUEST;
      if (res) toastr('The Client has been successfully removed.', 'success', 'Client removed');
  ...
  const getClientList = useMemo(
    () =>
      globalState?.getClientList?.result
        ? globalState?.getClientList?.result?.map((user: USER, index: number) => ({
            serial: index + 1,
            firstName: (
  ...
  return (
    <>
      ...
      <PageTitleWrapper>
        <PageHeader
          heading="Client Manager"
          content={<Typography variant="subtitle2">Manage your Client</Typography>}
          action={
            <>
            <Button
      ...
      <Drawer
        ...
        >
          ...
          <Typography variant="h6">{state.edit ? "Edit Record" : "Create New Record"}</Typography>
          {/* Example content */}
